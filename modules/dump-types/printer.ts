...
export class Printer {
  ...
  public static *printModule(def: MetadataModuleDefinition): Generator<string> {
    ...
    yield* def.functions
      .values()
      .map(Printer.printMethod)
      .map(Printer.functions)
      .map(Printer.exports)
      .map(Printer.suffix(";"));
    yield "";
  ...
  public static *printEnums(type: MetadataEnumDefinition): Generator<string> {
    ...
    yield* type.constants
      .map(Printer.printConstant)
      .map(Printer.suffix(","))
      .map(Printer.prefix(Printer.SPACING));
    yield "}";
  }
  ...
  public static *printClass(type: MetadataClassDefinition): Generator<string> {
    ...
    yield* type.constants
      .map(Printer.printProperty(Printer.printConstant))
      .map(Printer.publics)
      .map(prefix)
      .map(suffix);
    yield* type.properties
      .map(Printer.printProperty(Printer.printObject))
      .map(Printer.publics)
      .map(prefix)
      .map(suffix);
    ...
    if (type.iterator)
      yield suffix(
        prefix(
          `public [Symbol.iterator](): Iterator<${Printer.getType(type.iterator.optional_type ? type.iterator.optional_type : type.iterator)}>`,
        ),
      );
    yield "}";
  }
  ...
  public static printProperty<
    T extends
      | MetadataConstantDefinition
      | MetadataPropertyMemberDefinition
      | MetadataObjectDefinition,
  >(func: (param: T) => string): (param: T) => string {
    return (t) => {
      let code = func(t);
  ...
  public static printParams(types: MetadataFunctionArgumentDefinition[]): string {
    return types
      .map(
        (e) =>
          `${e.name}${e.details?.default_value !== undefined || e.type.optional_type ? "?" : ""}: ${Printer.getType(e.type.optional_type ? e.type.optional_type : e.type)}`,
      )
      .join(", ");
  }
  ...
  public static printObject(
    type: MetadataObjectDefinition | MetadataPropertyMemberDefinition,
  ): string {
  ...
  public static exports(text: string): string {
    return `export ${text}`;
  }
  public static consts(text: string): string {
    return `const ${text}`;
  }
  public static readonlies(text: string): string {
    return `readonly ${text}`;
  }
  public static statics(text: string): string {
    return `static ${text}`;
  }
  public static publics(text: string): string {
    return `public ${text}`;
  }
  public static functions(text: string): string {
  ...
  public static getType(
    ...
    switch (type.name) {
      case "array":
        return `Array<${type.element_type.name == "undefined" ? "never" : Printer.getType(type.element_type, isReturnType)}>`;
      case "boolean":
        return type.name;
      case "string":
        return type.name;
      ...
      case "optional":
        return safeContext
          ? `${Printer.getType(type.optional_type, isReturnType)} | undefined`
          : `(${Printer.getType(type.optional_type, isReturnType, true)} | undefined)`;
      case "promise":
        return `Promise<${Printer.getType(type.promise_type, isReturnType, true)}>`;
      case "iterator":
        return `IteratorResult<${Printer.getType(type.iterator_result, isReturnType, safeContext)}>`;
      case "variant":
        return safeContext
          ? `${type.variant_types.map((s) => Printer.getType(s, isReturnType, safeContext)).join(" | ")}`
          : `(${type.variant_types.map((s) => Printer.getType(s, isReturnType, true)).join(" | ")})`;
      case "map":
        return `Record<${Printer.getType(type.key_type, false, true)},${Printer.getType(type.value_type, isReturnType, true)}>`;
      case "closure":
        return `(${type.closure_type.argument_types.map((s, i) => `arg${i}${s.optional_type ? `?: ${Printer.getType(s.optional_type, false, true)}` : `: ${Printer.getType(s, false, true)}`}`).join(", ")})=>${Printer.getType(type.closure_type.return_type, true, false)}`;
      case "uint8":
      case "uint16":
      case "uint32":
      case "uint64":
