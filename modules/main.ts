...
} from "@bedrock-apis/bds-utils/links";
...
async function main(): Promise<number> {
  ...
  if ((failed = await GithubUtils.checkoutBranch(BRANCH_TO_UPDATE ?? "stable", true)))
    return failed;
  GithubUtils.clear();
  ...
  if (!link)
    throw new DumperError(
      ErrorCodes.UnavailableInstallationLink,
      `Link not available branch:${BRANCH_TO_UPDATE} platform:${platform}`,
    );
...
async function finialize(version: string): Promise<number> {
  ...
  let failed = 0;
  if (
    (failed = await Deno.writeTextFile(
      EXISTS_FILE,
      TO_JSON_FORMAT({
  ...
  if (
    (failed = await GithubUtils.commitAndPush(
      BRANCH_TO_UPDATE ?? "stable",
      "New Update - " + BASED_VERSION,
    ))
  )
    return failed;
