...
export class MetadataDumper {
  ...
  public static run(installation: Installation): Promise<number> {
    return this.CopyDocsTask(
      join(installation.directory, BDS_DOCS_FOLDER_NAME),
      OUTPUT_FOLDER,
    ).catch((_) => -1);
  }
  public static async CopyDocsTask(source: string, destination: string): Promise<number> {
    ...
    const action = async (fileName: string) => {
      ...
      if (fileName.endsWith(".json"))
        try {
          let object = JSON.parse(new TextDecoder().decode(data as Uint8Array)) as Record<
            string,
            any
          >;
    ...
    await Deno.writeTextFile(
      join(destination, CONTENTS_FILE_NAME),
      TO_JSON_FORMAT(contents.sort()),
    );
    return 0;
  }
}
